...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, ActivityIndicator, TextInput, Modal, BackHandler, Dimensions } from 'react-native';

// 獲取螢幕尺寸
const { height: screenHeight } = Dimensions.get('window');

// API 配置
const API_BASE_URL = 'http://192.168.68.103:3004/api';
...
const apiService = {
  ...
  // 狀態管理 API (按照後端路由設計)
  updateOrderStatus: async (token: string, orderId: string, statusData: { primaryStatus: string; secondaryStatus: string }) => {
    try {
  ...
  updatePickupStatus: async (token: string, orderId: string, statusData: { primaryStatus: string; secondaryStatus: string }) => {
    try {
      const response = await fetch(`${API_BASE_URL}/status/orders/${orderId}/status/pickup`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  updateDeliveryStatus: async (token: string, orderId: string, statusData: { status: string; time?: string; address?: string; po?: string; deliveredBy?: string }) => {
    try {
      const response = await fetch(`${API_BASE_URL}/status/orders/${orderId}/status/delivery`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  updateCheckStatus: async (token: string, orderId: string, statusData: { status: string }) => {
    try {
      const response = await fetch(`${API_BASE_URL}/status/orders/${orderId}/status/check`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  // 獲取專案狀態
  getProjectStatus: async (token: string, projectId: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/status/projects/${projectId}/status`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      return await response.json();
    } catch (error) {
...
export default function App() {
  ...
  const [selectedOrderMaterials, setSelectedOrderMaterials] = useState(null);
  ...
  const loadOrdersByRole = async (token: string, userRole: string) => {
    ...
    try {
      const allOrders = await apiService.getAllOrders(token);
      console.log('All orders loaded:', allOrders);
      
      // 為每個訂單添加模擬的狀態數據 (將來從狀態API獲取)
      const ordersWithStatus = allOrders.map(order => ({
        ...order,
        // 模擬狀態數據 - 將來從 /api/status/{projectId} 獲取
        orderStatus: {
        ...
        deliveryStatus: {
          ...
          address: order.id.includes('2120') ? '台北市信義區' : '',
          po: order.id.includes('2120') ? 'PO-2024-001' : '',
          deliveredBy: order.id.includes('2120') ? 'Mark' : ''
        },
        checkStatus: {
  ...
  // 狀態管理處理函數
  const handleStatusUpdate = (orderId: string, statusType: string) => {
    ...
    if (order) {
      switch (statusType) {
        case 'ORDER':
          setStatusFormData({
            primaryStatus: order.orderStatus?.primaryStatus || '',
            secondaryStatus: order.orderStatus?.secondaryStatus || '',
            deliveryTime: '',
            deliveryAddress: '',
            deliveryPO: '',
            deliveredBy: ''
        ...
        case 'PICKUP':
          setStatusFormData({
            primaryStatus: order.pickupStatus?.primaryStatus || '',
            secondaryStatus: order.pickupStatus?.secondaryStatus || '',
            deliveryTime: '',
            deliveryAddress: '',
            deliveryPO: '',
            deliveredBy: ''
        ...
        case 'DELIVERY':
          setStatusFormData({
            primaryStatus: order.deliveryStatus?.status || '',
            secondaryStatus: '',
            deliveryTime: order.deliveryStatus?.time || '',
            deliveryAddress: order.deliveryStatus?.address || '',
            deliveryPO: order.deliveryStatus?.po || '',
            deliveredBy: order.deliveryStatus?.deliveredBy || ''
          });
          break;
        case 'CHECK':
          setStatusFormData({
            primaryStatus: order.checkStatus?.status || '',
            secondaryStatus: '',
            deliveryTime: '',
            deliveryAddress: '',
            deliveryPO: '',
  ...
  const handleShowMaterialDetails = (order: any) => {
    setSelectedOrderMaterials(order);
  ...
  const handleCloseMaterialModal = () => {
    ...
    setSelectedOrderMaterials(null);
  };

  // 狀態選擇處理
  const handleStatusOptionSelect = (field: string, value: string) => {
  ...
  const handleSubmitStatusUpdate = async () => {
    if (!selectedOrder || !selectedStatusType) {
      Alert.alert('錯誤', '請選擇要更新的狀態');
      return;
    }
    ...
    try {
      ...
      switch (selectedStatusType) {
        case 'ORDER':
          result = await apiService.updateOrderStatus(authToken, selectedOrder, {
            primaryStatus: statusFormData.primaryStatus,
            secondaryStatus: statusFormData.secondaryStatus
          });
          break;
          
        case 'PICKUP':
          result = await apiService.updatePickupStatus(authToken, selectedOrder, {
            primaryStatus: statusFormData.primaryStatus,
            secondaryStatus: statusFormData.secondaryStatus
          });
          break;
        ...
        case 'DELIVERY':
          result = await apiService.updateDeliveryStatus(authToken, selectedOrder, {
            status: statusFormData.primaryStatus,
            time: statusFormData.deliveryTime,
            address: statusFormData.deliveryAddress,
            po: statusFormData.deliveryPO,
            deliveredBy: statusFormData.deliveredBy
          });
          break;
          
        case 'CHECK':
          result = await apiService.updateCheckStatus(authToken, selectedOrder, {
            status: statusFormData.primaryStatus
          });
          break;
          
        default:
          throw new Error('未知的狀態類型');
      }
      
      if (result.success) {
        Alert.alert('成功', '狀態更新成功');
        setShowStatusModal(false);
        // 重新載入資料
        await loadInitialData(authToken);
      } else {
        Alert.alert('錯誤', result.message || '狀態更新失敗');
      }
    } catch (error) {
      console.error('狀態更新錯誤:', error);
      Alert.alert('錯誤', '狀態更新時發生錯誤');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!isLoggedIn) {
    return (
      <View style={styles.loginContainer}>
        <ScrollView style={styles.loginScrollView} contentContainerStyle={{ paddingBottom: 50 }}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (feature.includes('輔材訂單管理')) {
                  ...
                  setCurrentPage('status-management');
    ...
    <View style={styles.pageContainer}>
      <View style={styles.pageHeader}>
        ...
        <View style={styles.connectionStatus}>
          <Text style={styles.connectionText}>🟢 已連線</Text>
        </View>
      </View>
      ...
      >
        {loading ? (
          ...
          orders.map((order, index) => (
            <View key={order.id || index} style={styles.orderStatusCard}>
              {/* 訂單基本信息 */}
                ...
                <View style={styles.orderHeader}>
                  ...
                  <View style={styles.orderTypeBadge}>
                    <Text style={styles.orderTypeText}>
                      {order.type === 'FINISHED' || order.type === 'finished' ? 'AM完成材' : 'PM輔材'}
                    </Text>
                  </View>
                </View>
                
                <Text style={styles.orderName}>
                  {order.type === 'FINISHED' || order.type === 'finished' ? '完成材訂單' : '輔材訂單'} - {order.id}
                </Text>
                
                <Text style={styles.orderCreator}>
                  創建者: {order.createdBy || order.userId || '未知'}
                </Text>
                
                {/* 材料清單 - 緊湊顯示 */}
                <TouchableOpacity 
                  style={styles.materialsList}
                  onPress={() => handleShowMaterialDetails(order)}
                >
                  <Text style={styles.materialsTitle}>材料清單 ({order.items?.length || 0}項):</Text>
                  <Text style={styles.materialsPreview} numberOfLines={2}>
                    {order.items?.map(item => `${item.materialName} x${item.quantity}`).join(', ') || '無材料信息'}
                  </Text>
                  <Text style={styles.materialsMore}>點擊查看詳細 →</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.compactStatusGrid}>
                {/* 叫貨狀態 */}
                <TouchableOpacity 
                  style={styles.compactStatusItem}
                  onPress={() => handleStatusUpdate(order.id, 'ORDER')}
                >
                  <View style={[styles.statusBadge, styles.orderBadge]}>
                    <Text style={styles.statusBadgeText}>叫貨狀態</Text>
                  </View>
                  <Text style={styles.statusValue}>{order.orderStatus?.primaryStatus || '未設定'}</Text>
                  <Text style={styles.statusSubValue}>{order.orderStatus?.secondaryStatus || ''}</Text>
                </TouchableOpacity>

                {/* 取貨狀態 */}
                <TouchableOpacity 
                  style={styles.compactStatusItem}
                  onPress={() => handleStatusUpdate(order.id, 'PICKUP')}
                >
                  <View style={[styles.statusBadge, styles.pickupBadge]}>
                    <Text style={styles.statusBadgeText}>取貨狀態</Text>
                  </View>
                  <Text style={styles.statusValue}>{order.pickupStatus?.primaryStatus || '未設定'}</Text>
                  <Text style={styles.statusSubValue}>{order.pickupStatus?.secondaryStatus || ''}</Text>
                </TouchableOpacity>

                {/* 到案狀態 */}
                <TouchableOpacity 
                  style={styles.compactStatusItem}
                  onPress={() => handleStatusUpdate(order.id, 'DELIVERY')}
                >
                  <View style={[styles.statusBadge, styles.deliveryBadge]}>
                    <Text style={styles.statusBadgeText}>到案狀態</Text>
                  </View>
                  <Text style={styles.statusValue}>{order.deliveryStatus?.status || '未設定'}</Text>
                  <Text style={styles.statusSubValue}>{order.deliveryStatus?.time ? new Date(order.deliveryStatus.time).toLocaleTimeString('zh-TW', {hour: '2-digit', minute: '2-digit'}) : ''}</Text>
                </TouchableOpacity>

                {/* 點收狀態 */}
                <TouchableOpacity 
                  style={styles.compactStatusItem}
                  onPress={() => handleStatusUpdate(order.id, 'CHECK')}
                >
                  <View style={[styles.statusBadge, styles.checkBadge]}>
                    <Text style={styles.statusBadgeText}>點收狀態</Text>
                  </View>
                  <Text style={styles.statusValue}>{order.checkStatus?.status || '未設定'}</Text>
                  <Text style={styles.statusSubValue}>{order.checkStatus?.status ? '✅完成' : ''}</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))
  ...
  return (
    <View style={styles.app}>
      ...
      {currentPage === 'status-management' && renderStatusManagement()}
      ...
      <Modal visible={showStatusModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.statusModalContent}>
            ...
            <View style={styles.statusUpdateForm}>
              {selectedStatusType === 'ORDER' && (
                <View>
                  <Text style={styles.formLabel}>主要狀態</Text>
                  <View style={styles.statusOptions}>
                    <TouchableOpacity 
                      style={[styles.statusOption, statusFormData.primaryStatus === '' && styles.selectedOption]}
                      onPress={() => handleStatusOptionSelect('primaryStatus', '')}
                    >
                      <Text style={statusFormData.primaryStatus === '' ? styles.selectedOptionText : styles.statusOptionText}>空白</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                      style={[styles.statusOption, statusFormData.primaryStatus === 'Ordered' && styles.selectedOption]}
                      onPress={() => handleStatusOptionSelect('primaryStatus', 'Ordered')}
                    >
                      <Text style={statusFormData.primaryStatus === 'Ordered' ? styles.selectedOptionText : styles.statusOptionText}>Ordered</Text>
                    </TouchableOpacity>
                  </View>
                  
                  {statusFormData.primaryStatus === 'Ordered' && (
                    <>
                      <Text style={styles.formLabel}>處理狀態</Text>
                      <View style={styles.statusOptions}>
                        <TouchableOpacity 
                          style={[styles.statusOption, statusFormData.secondaryStatus === 'Processing' && styles.selectedOption]}
                          onPress={() => handleStatusOptionSelect('secondaryStatus', 'Processing')}
                        >
                          <Text style={statusFormData.secondaryStatus === 'Processing' ? styles.selectedOptionText : styles.statusOptionText}>Processing</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                          style={[styles.statusOption, statusFormData.secondaryStatus === 'waiting for pick' && styles.selectedOption]}
                          onPress={() => handleStatusOptionSelect('secondaryStatus', 'waiting for pick')}
                        >
                          <Text style={statusFormData.secondaryStatus === 'waiting for pick' ? styles.selectedOptionText : styles.statusOptionText}>waiting for pick</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                          style={[styles.statusOption, statusFormData.secondaryStatus === 'pending' && styles.selectedOption]}
                          onPress={() => handleStatusOptionSelect('secondaryStatus', 'pending')}
                        >
                          <Text style={statusFormData.secondaryStatus === 'pending' ? styles.selectedOptionText : styles.statusOptionText}>pending</Text>
                        </TouchableOpacity>
                      </View>
                    </>
                  )}
              ...
              {selectedStatusType === 'PICKUP' && (
                <View>
                  <Text style={styles.formLabel}>取貨狀態</Text>
                  <View style={styles.statusOptions}>
                    <TouchableOpacity 
                      style={[styles.statusOption, statusFormData.primaryStatus === 'Picked' && styles.selectedOption]}
                      onPress={() => handleStatusOptionSelect('primaryStatus', 'Picked')}
                    >
                      <Text style={statusFormData.primaryStatus === 'Picked' ? styles.selectedOptionText : styles.statusOptionText}>Picked</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                      style={[styles.statusOption, statusFormData.primaryStatus === 'Failed' && styles.selectedOption]}
                      onPress={() => handleStatusOptionSelect('primaryStatus', 'Failed')}
                    >
                      <Text style={statusFormData.primaryStatus === 'Failed' ? styles.selectedOptionText : styles.statusOptionText}>Failed</Text>
                    </TouchableOpacity>
                  </View>
                  
                  {statusFormData.primaryStatus && (
                    <>
                      <Text style={styles.formLabel}>處理結果</Text>
                      <View style={styles.statusOptions}>
                        {statusFormData.primaryStatus === 'Picked' ? (
                          <>
                            <TouchableOpacity 
                              style={[styles.statusOption, statusFormData.secondaryStatus === '(B.T.W)' && styles.selectedOption]}
                              onPress={() => handleStatusOptionSelect('secondaryStatus', '(B.T.W)')}
                            >
                              <Text style={statusFormData.secondaryStatus === '(B.T.W)' ? styles.selectedOptionText : styles.statusOptionText}>(B.T.W)</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                              style={[styles.statusOption, statusFormData.secondaryStatus === '(D.T.S)' && styles.selectedOption]}
                              onPress={() => handleStatusOptionSelect('secondaryStatus', '(D.T.S)')}
                            >
                              <Text style={statusFormData.secondaryStatus === '(D.T.S)' ? styles.selectedOptionText : styles.statusOptionText}>(D.T.S)</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                              style={[styles.statusOption, statusFormData.secondaryStatus === '(B.T.W/MP)' && styles.selectedOption]}
                              onPress={() => handleStatusOptionSelect('secondaryStatus', '(B.T.W/MP)')}
                            >
                              <Text style={statusFormData.secondaryStatus === '(B.T.W/MP)' ? styles.selectedOptionText : styles.statusOptionText}>(B.T.W/MP)</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                              style={[styles.statusOption, statusFormData.secondaryStatus === '(D.T.S/MP)' && styles.selectedOption]}
                              onPress={() => handleStatusOptionSelect('secondaryStatus', '(D.T.S/MP)')}
                            >
                              <Text style={statusFormData.secondaryStatus === '(D.T.S/MP)' ? styles.selectedOptionText : styles.statusOptionText}>(D.T.S/MP)</Text>
                            </TouchableOpacity>
                          </>
                        ) : (
                          <>
                            <TouchableOpacity 
                              style={[styles.statusOption, statusFormData.secondaryStatus === '(E.S)' && styles.selectedOption]}
                              onPress={() => handleStatusOptionSelect('secondaryStatus', '(E.S)')}
                            >
                              <Text style={statusFormData.secondaryStatus === '(E.S)' ? styles.selectedOptionText : styles.statusOptionText}>(E.S)</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                              style={[styles.statusOption, statusFormData.secondaryStatus === '(E.H)' && styles.selectedOption]}
                              onPress={() => handleStatusOptionSelect('secondaryStatus', '(E.H)')}
                            >
                              <Text style={statusFormData.secondaryStatus === '(E.H)' ? styles.selectedOptionText : styles.statusOptionText}>(E.H)</Text>
                            </TouchableOpacity>
                          </>
                        )}
                      </View>
              ...
              >
                {loading ? (
                  ...
                  <Text style={styles.updateButtonText}>更新狀態</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
      ...
      <Modal visible={showMaterialModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.materialModalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                材料詳情 - #{selectedOrderMaterials?.id}
              </Text>
            ...
            <ScrollView style={styles.materialDetailsList}>
              {selectedOrderMaterials?.items?.map((item, index) => (
                <View key={index} style={styles.materialDetailCard}>
                  <View style={styles.materialDetailInfo}>
                    <Text style={styles.materialDetailName}>{item.materialName}</Text>
                    <Text style={styles.materialDetailQuantity}>數量: {item.quantity}</Text>
                    <Text style={styles.materialDetailPrice}>單價: CAD ${item.price || 0}</Text>
                  </View>
                  <View style={styles.materialImagePlaceholder}>
                    <Text style={styles.materialImageText}>📷</Text>
                    <Text style={styles.materialImageLabel}>圖片</Text>
                  </View>
                </View>
              )) || (
                <View style={styles.emptyMaterials}>
                  <Text style={styles.emptyMaterialsText}>此訂單暫無材料信息</Text>
                </View>
              )}
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  // 緊湊訂單狀態管理樣式
  connectionStatus: { alignItems: 'center' },
  connectionText: { fontSize: 12, color: '#28a745', fontWeight: '600' },
  ...
  // 訂單基本信息
  orderBasicInfo: { marginBottom: 16 },
  orderHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  orderNumber: { fontSize: 18, fontWeight: 'bold', color: '#007bff' },
  orderTypeBadge: { backgroundColor: '#f8f9fa', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: '#dee2e6' },
  orderTypeText: { fontSize: 12, fontWeight: '600', color: '#495057' },
  orderName: { fontSize: 16, fontWeight: '600', color: '#333', marginBottom: 4 },
  orderCreator: { fontSize: 14, color: '#666', marginBottom: 8 },
  
  // 材料清單
  materialsList: { backgroundColor: '#f8f9fa', borderRadius: 8, padding: 12, borderWidth: 1, borderColor: '#dee2e6' },
  materialsTitle: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 4 },
  materialsPreview: { fontSize: 13, color: '#666', lineHeight: 18, marginBottom: 4 },
  materialsMore: { fontSize: 12, color: '#007bff', fontWeight: '600' },
  ...
  orderBadge: { backgroundColor: '#007bff' },
  pickupBadge: { backgroundColor: '#28a745' },
  deliveryBadge: { backgroundColor: '#ffc107' },
  checkBadge: { backgroundColor: '#dc3545' },
  statusBadgeText: { fontSize: 10, fontWeight: 'bold', color: '#fff' },
  statusValue: { fontSize: 12, fontWeight: '600', color: '#333', textAlign: 'center', marginBottom: 2 },
  statusSubValue: { fontSize: 10, color: '#666', textAlign: 'center' },
  ...
  // 狀態更新表單
  statusUpdateForm: { flex: 1 },
  formLabel: { fontSize: 16, fontWeight: '600', color: '#333', marginBottom: 8, marginTop: 16 },
  ...
  selectedOptionText: { fontSize: 14, color: '#fff' },
  
  // 材料詳情
  materialDetailsList: { flex: 1, marginTop: 16 },
  materialDetailCard: { 
    flexDirection: 'row', 
    backgroundColor: '#f8f9fa', 
    borderRadius: 8, 
  ...
  materialDetailInfo: { flex: 1 },
  materialDetailName: { fontSize: 16, fontWeight: '600', color: '#333', marginBottom: 4 },
  materialDetailQuantity: { fontSize: 14, color: '#666', marginBottom: 2 },
  materialDetailPrice: { fontSize: 14, color: '#28a745', fontWeight: '600' },
  ...
  materialImageText: { fontSize: 24 },
  materialImageLabel: { fontSize: 10, color: '#666', marginTop: 2 },
  emptyMaterials: { alignItems: 'center', marginTop: 40 },
  emptyMaterialsText: { fontSize: 16, color: '#666' },
  
  updateButton: { backgroundColor: '#007bff', borderRadius: 8, padding: 16, alignItems: 'center', marginTop: 20 },
