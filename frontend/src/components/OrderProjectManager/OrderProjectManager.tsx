  ...
  const loadProjects = async () => {
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        // 只顯示活躍的專案，智能過濾自動創建的專案
        const activeProjects = response.data.filter(p => {
          if (p.overallStatus !== 'ACTIVE') return false;
          
          // 檢查是否為自動生成的專案名稱格式
          const isAutoGenerated = (
            p.projectName.match(/^輔材專案-\d{4}-\d{2}-\d{2}-(order-|id-)\d+$/) ||
            p.projectName.match(/^成品專案-\d{4}-\d{2}-\d{2}-(order-|id-)\d+$/)
          );
          
          // 如果不是自動生成格式，則顯示（包括重命名的專案）
          if (!isAutoGenerated) return true;
          
          // 如果是自動生成格式，但有手動描述，也顯示
          if (p.description && p.description !== '自動創建的專案' && p.description.trim() !== '') {
            return true;
          }
