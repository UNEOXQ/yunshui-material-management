...
export class MaterialModel {
  ...
  static async findByType(type: MaterialType): Promise<Material[]> {
    try {
      const result = await memoryDb.getAllMaterials({ type });
      return result.materials;
    } catch (error) {
      console.warn('Memory database failed, trying PostgreSQL:', error);
      
      const query = 'SELECT * FROM materials WHERE type = $1 ORDER BY created_at DESC';
      const result = await pool.query(query, [type]);
      return result.rows.map(row => this.entityToModel(row));
    }
  }
  ...
  static async findByCategory(category: string): Promise<Material[]> {
    try {
      const result = await memoryDb.getAllMaterials({ category });
      return result.materials;
    } catch (error) {
      console.warn('Memory database failed, trying PostgreSQL:', error);
      
      const query = 'SELECT * FROM materials WHERE category = $1 ORDER BY created_at DESC';
  ...
  static async getSuppliers(type?: MaterialType): Promise<string[]> {
    try {
      ...
      let query = 'SELECT DISTINCT supplier FROM materials WHERE supplier IS NOT NULL AND supplier != \'\'';
      const params: any[] = [];
